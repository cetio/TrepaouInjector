...
namespace ManualMapInjection
{
    public class LoadLib
    {
        ...
        public static bool Inject(Process proc, string path)
        {
            Init();
            ...
            hGame = OpenProcess(ProcessAccessFlags.All, false, proc.Id);
            if (hGame == IntPtr.Zero)
                return false;

            BypassCSGOHook();
            InjectDLL(proc.Id, path);
            RestoreCSGOHook();
        ...
        private static void InjectDLL(int pid, string path)
        {
